...
# 课纲
SYL_MATH8_ID  = "g0000000-0000-0000-0000-000000000001"
SYL_CHN7_ID   = "g0000000-0000-0000-0000-000000000002"
SYL_ENG9_ID   = "g0000000-0000-0000-0000-000000000003"
SYL_PHY8_ID   = "g0000000-0000-0000-0000-000000000004"
...
async def run():
    ...
    async with factory() as db:
        ...
        await step7_papers(db)
        await step8_submissions(db)
        await step9_notebooks(db)
        await step10_selfstudy(db)
        await step11_notifications(db)
        await step12_parent_module(db)
        await step13_topic_board(db)
        await step14_recommendations(db)
        await step15_tasks(db)
        summary()
...
async def step0_clear(db: AsyncSession):
    ...
    delete_order = [
        ...
        "answer_submissions", "exam_paper_questions",
    ...
    for sid, title, grade, prov, subj, content, creator in syllabi:
        await db.execute(text("""
            INSERT INTO syllabi (id, title, grade_level, province, subject, content,
                knowledge_tree, status, version, is_current, created_by, created_at, updated_at)
            VALUES (:id, :title, CAST(:grade AS jsonb), :prov, :subj, CAST(:content AS jsonb),
                CAST('{}' AS jsonb), 'PUBLISHED', 1, true, :creator, now(), now())
...
async def step7_papers(db: AsyncSession):
    print("[7/15] 导入试卷及题目关联...")
    papers = [
        (PAPER_MATH_MID_ID,  "八年级数学上册期中测试", "实数+代数式+方程三章，满分100分",
         "数学", json.dumps({"scope":"grade","grades":["G8"]}), 100, 120,
         "请将答案写在答题纸上，计算题须写出完整解题步骤，否则不得分。", T_MATH_ID),
        (PAPER_MATH_UNIT_ID, "八年级数学·第一章单元测试(实数)", "实数概念与运算，满分50分",
         "数学", json.dumps({"scope":"grade","grades":["G8"]}), 50, 60,
         "全部题目均需作答。", T_MATH_ID),
        (PAPER_MATH_FINAL_ID,"八年级数学上册期末测试", "全册综合检测，满分120分",
         "数学", json.dumps({"scope":"grade","grades":["G8"]}), 120, 120,
         "认真审题，书写规范。选择题用2B铅笔填涂。", T_MATH_ID),
        (PAPER_CHN_MID_ID,   "七年级语文上册期中检测", "现代文+古诗词+写作，满分100分",
         "语文", json.dumps({"scope":"grade","grades":["G7"]}), 100, 120,
         "作文字迹工整，卷面整洁将酌情加分。", T_CHINESE_ID),
        (PAPER_ENG_FINAL_ID, "九年级英语中考模拟卷", "单选+填空+阅读+写作，满分120分",
         "英语", json.dumps({"scope":"grade","grades":["G9"]}), 120, 120,
         "听力部分另行安排。本卷考查语法、阅读和写作能力。", T_ENG_ID),
        (PAPER_PHY_UNIT_ID,  "八年级物理·光现象单元测试", "光的传播+反射+折射，满分60分",
         "物理", json.dumps({"scope":"grade","grades":["G8"]}), 60, 45,
         "作图题请用铅笔和直尺。", T_PHY_ID),
    ]
    for pid, title, desc, subj, grade, score, dur, instr, creator in papers:
        await db.execute(text("""
            ...
            VALUES (:id, :title, :desc, :subj, CAST(:grade AS jsonb), 'PUBLISHED',
                :score, :dur, :instr, :creator, now(), now())
        """), {"id": pid, "title": title, "desc": desc, "subj": subj,
               "grade": grade, "score": score, "dur": dur, "instr": instr, "creator": creator})

    # 试卷-题目关联
    paper_q_map = {
        PAPER_MATH_MID_ID: [
            ("q1",1,3),("q2",2,3),("q3",3,3),("q4",4,3),("q5",5,3),("q6",6,3),
            ("q7",7,5),("q8",8,3),("q9",9,3),("q13",10,6),("q14",11,6),
            ("q15",12,6),("q18",13,4),("q19",14,4),("q20",15,6),
            ("q24",16,8),("q25",17,10),("q28",18,10),
        ],
        PAPER_MATH_UNIT_ID: [
            ("q1",1,3),("q5",2,3),("q7",3,5),("q9",4,3),("q10",5,3),
            ("q14",6,6),("q18",7,4),("q19",8,4),("q21",9,4),("q24",10,8),("q30",11,6),
        ],
        PAPER_MATH_FINAL_ID: [
            ("q1",1,3),("q3",2,3),("q4",3,3),("q6",4,3),("q7",5,5),
            ("q10",6,3),("q12",7,3),("q13",8,6),("q14",9,6),
            ("q16",10,8),("q17",11,6),("q20",12,6),("q22",13,4),
            ("q23",14,4),("q25",15,10),("q27",16,12),("q28",17,10),
            ("q29",18,4),("q30",19,6),
        ],
        PAPER_CHN_MID_ID: [
            ("q31",1,2),("q32",2,2),("q33",3,2),("q34",4,2),("q35",5,2),("q36",6,2),
            ("q37",7,2),("q38",8,2),("q39",9,3),("q40",10,4),("q41",11,3),
            ("q42",12,3),("q43",13,8),("q44",14,6),("q47",15,4),
            ("q49",16,8),("q45",17,20),
        ],
        PAPER_ENG_FINAL_ID: [
            ("q51",1,3),("q52",2,3),("q53",3,3),("q54",4,2),("q55",5,2),
            ("q56",6,3),("q57",7,3),("q58",8,2),("q59",9,2),("q60",10,2),
            ("q61",11,4),("q62",12,4),("q63",13,3),("q64",14,4),
            ("q65",15,6),("q66",16,10),("q67",17,6),("q68",18,12),
            ("q69",19,8),("q70",20,6),
        ],
        PAPER_PHY_UNIT_ID: [
            ("q71",1,3),("q72",2,3),("q73",3,3),("q74",4,3),("q75",5,3),
            ("q76",6,3),("q77",7,6),("q78",8,6),("q79",9,3),("q80",10,4),
        ],
    }

    total_eqs = 0
    for pid, qlist in paper_q_map.items():
        for qkey, pos, sc in qlist:
            await db.execute(text("""
                INSERT INTO exam_paper_questions (id, exam_paper_id, question_id, position, score)
                VALUES (:id, :pid, :qid, :pos, :sc)
            """), {"id": _uid(), "pid": pid, "qid": Q[qkey], "pos": pos, "sc": sc})
            total_eqs += 1
    await db.commit()
    print(f"   导入 {len(papers)} 份试卷, {total_eqs} 条题目关联\n")
