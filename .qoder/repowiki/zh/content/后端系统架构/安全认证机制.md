...
- [backend/app/core/security.py:13-104](file://backend/app/core/security.py#L13-L104)
- [backend/app/api/v1/endpoints/auth_v2.py:23-476](file://backend/app/api/v1/endpoints/auth_v2.py#L23-L476)
...
- [backend/app/core/security.py:43-96](file://backend/app/core/security.py#L43-L96)
...
图表来源
- [backend/app/core/security.py:64-96](file://backend/app/core/security.py#L64-L96)
...
```mermaid
flowchart TD
PStart(["接收明文密码"]) --> Hash["使用 bcrypt 生成哈希"]
...
- 刷新失败
  - 刷新令牌无效或已过期；前端将清除本地令牌并跳转登录页，需重新登录。

章节来源
- [backend/app/core/security.py:68-95](file://backend/app/core/security.py#L68-L95)
...
## 结论
系统实现了基于 JWT 的多角色认证方案，结合图形验证码与短信验证提升了登录安全性；前端通过拦截器与状态管理实现了透明的令牌续签与会话维持。建议在生产环境中强化 CSRF 与 XSS 防护，并对认证路径引入缓存与速率限制以提升整体安全与性能。
