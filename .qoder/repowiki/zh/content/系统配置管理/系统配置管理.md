...
## 更新摘要
**所做更改**
- 更新了数据库命名约定部分，反映从 'edu_system_bak' 到 'edu_system' 的回退
- 更新了配置文件结构说明，包含最新的数据库配置
- 更新了启动脚本分析，反映数据库名称的正确配置
- 增强了配置验证和故障排除指南
...
### 配置文件结构与环境变量管理
- sysconfig.json 结构要点：
  - database：server、port、database（当前为 'edu_system'）、user
...
### 数据库命名约定调整
**更新** 数据库名称从 'edu_system_bak' 回退到 'edu_system'

系统中的数据库名称已从 'edu_system_bak' 回退到 'edu_system'，这一变化体现在多个配置文件中：

- **sysconfig.json**：数据库配置项明确指定为 "edu_system"
- **启动脚本**：start.sh 中的数据库名称读取和默认值均为 'edu_system'
- **配置服务**：默认数据库名称为 'edu_system'
- **演示数据脚本**：数据库连接字符串使用 'edu_system'
- **数据库管理API**：默认数据库名称为 'edu_system'

```mermaid
flowchart TD
Start(["数据库配置"]) --> Check["检查 sysconfig.json"]
Check --> Found{"找到数据库配置?"}
Found --> |是| Read["读取数据库名称"]
Found --> |否| Default["使用默认值 'edu_system'"]
Read --> Validate["验证数据库名称"]
Validate --> Apply["应用到连接字符串"]
Default --> Apply
Apply --> End(["配置完成"])
```

**图表来源**
- [backend/sysconfig.json:5](file://backend/sysconfig.json#L5)
- [start.sh:54](file://start.sh#L54)
- [backend/app/core/config.py:18](file://backend/app/core/config.py#L18)

**章节来源**
- [backend/sysconfig.json:5](file://backend/sysconfig.json#L5)
- [start.sh:54](file://start.sh#L54)
- [backend/app/core/config.py:18](file://backend/app/core/config.py#L18)
- [backend/demo_data.py:41](file://backend/demo_data.py#L41)
- [backend/app/api/v1/endpoints/database.py:91](file://backend/app/api/v1/endpoints/database.py#L91)
...
- 存储异常：
  ...
  - 确认上传目录权限
- 数据库连接问题：
  - **更新** 确认数据库名称为 'edu_system' 而非 'edu_system_bak'
  - 检查 PostgreSQL 服务状态和端口可达性
  - 验证数据库用户权限和密码
- 健康检查：
...
## 结论
本配置体系通过"环境变量优先 + JSON 非敏感参数 + 运行时注入"的设计，在保证安全性的前提下实现了灵活的多环境支持与动态配置管理。结合 Docker 编排与代理机制，开发与生产的配置差异得以最小化，同时保留了足够的扩展空间。数据库命名约定的标准化（'edu_system'）确保了系统的稳定性和一致性。
...
- **更新** 确保数据库名称一致性，使用 'edu_system' 而非 'edu_system_bak'

### 配置迁移策略
- 从旧版本升级时，先备份 sysconfig.json，再逐步迁移新增字段
- 通过配置 API 逐项更新，避免一次性大规模变更
- 在测试环境先行验证，再推广至生产
- **更新** 如发现数据库名称为 'edu_system_bak'，应立即更新为 'edu_system'

### 数据库配置验证清单
- [ ] sysconfig.json 中数据库名称为 'edu_system'
- [ ] 启动脚本中数据库名称读取为 'edu_system'
- [ ] 配置服务默认数据库名称为 'edu_system'
- [ ] 演示数据脚本连接字符串使用 'edu_system'
- [ ] 数据库管理API默认数据库名称为 'edu_system'
- [ ] PostgreSQL 实际数据库名为 'edu_system'