...
## 项目结构
该仓库采用前后端分离的双栈结构：后端基于 FastAPI + PostgreSQL（通过 Alembic 进行迁移），前端基于 Vite + React；同时提供一键启动脚本与 Docker Compose 编排，便于本地开发与演示。
...
- 一键启动脚本
  - 自动创建/校验 Conda 环境、安装依赖、检查 PostgreSQL、执行 Alembic 迁移、初始化种子数据与系统管理员、启动后端与前端并等待健康检查。
...
- 数据库与迁移
  ...
  - Alembic 在线/离线迁移，根据 settings.DATABASE_URL 动态注入真实数据库连接。
- 健康检查
...
- 迁移入口
  - alembic.ini 定义日志与格式；env.py 动态注入 DATABASE_URL 并选择在线/离线迁移。
- 模型与命名
...
BuildURL --> ChooseMode{"迁移模式？"}
ChooseMode --> |在线| ConnectDB["创建真实数据库连接"]
ChooseMode --> |离线| Offline["使用配置文件中的 URL"]
ConnectDB --> RunMigs["执行迁移"]
Offline --> RunMigs
RunMigs --> Done(["完成"])
```
...
### 一键启动脚本（start.sh）
- 功能概览
  - 初始化 Conda 环境、安装后端依赖、检查并创建数据库、执行 Alembic 迁移、播种参考数据与系统管理员、启动后端与前端并等待健康检查。
- 关键流程
  - 读取 sysconfig.json 获取数据库参数，设置 PGPASSWORD。
  - 通过 psql 创建数据库并执行迁移；若迁移失败则回退到直接建表。
...
- 启动失败
  ...
  - 若 Alembic 迁移失败，回退到直接建表逻辑或修正迁移脚本。
- 数据库问题
...
## 结论
本项目提供了清晰的容器化与一键启动能力，结合 Alembic 迁移与统一配置体系，具备良好的可运维性。建议在生产环境中完善 CI/CD、监控告警、备份与灾备、安全加固与变更控制流程，以满足企业级上线要求。
